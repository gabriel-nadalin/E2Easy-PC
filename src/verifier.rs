...
impl<G: Group> Verifier<G> {
    ...
    pub fn check_proof(&self, pi: &ShuffleProof<G>, e_list: &Vec<G::Element>, e_prime_list: &Vec<G::Element>) -> bool {
        ...
        let e_prime = e_list.iter()
            .zip(u_list.iter())
            .map(|(e, u)| e.mul_scalar(u))
            .fold(self.group.identity(), |acc, x| acc.add(&x));
        ...
        let y = (e_list, e_prime_list, c_list, c_hat_list.clone());
        // IMPORTANTE
        // TODO: definir forma canonica de serializacao para hash com formato consistente
        ...
        //     .fold(self.group.identity(), |acc, x| acc.add(&x))
        // );
        let t_prime_3 = e_prime.mul_scalar(&c).inv().add(&self.group.mul_generator(&s.3).inv()).add(
            &e_prime_list.iter()
            .zip(s.5.iter())
            .map(|(e, s_prime)| e.mul_scalar(s_prime))
            .fold(self.group.identity(), |acc, x| acc.add(&x))
        );
        ...
        for i in 0..self.n {
            if i == 0 {
                t_hat_prime_list.push(c_hat_list[i].mul_scalar(&c).inv().add(&self.group.mul_generator(&s.4[i])).add(&self.h_list[0].mul_scalar(&s.5[i])));
            } else {
                t_hat_prime_list.push(c_hat_list[i].mul_scalar(&c).inv().add(&self.group.mul_generator(&s.4[i])).add(&&c_hat_list[i-1].mul_scalar(&s.5[i])));
            }
        }
